...
class ProductControllerTest {
        ...
        void setup() {
                ...
                req = ProductRequestDTO.builder().name("Phone").description("Smart").price(10.0).stock(5)
                                .categories(List.of())
                                .build();
        ...
        void getAllProductImages_returnsList() throws Exception {
                given(productService.getProductImages(productId))
                                .willReturn(List.of(ImageDTO.builder().name("a").build()));
                mockMvc.perform(get("/api/v1/products/{id}/productImages", productId))
                                .andDo(print())
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.size()", CoreMatchers.is(1)));
                verify(productService, times(1)).getProductImages(productId);
        }
        ...
        void getProductReviews_returnsList() throws Exception {
                given(productService.getReviews(productId))
                                .willReturn(List.of(ReviewResponseDTO.builder().id(1).build()));
